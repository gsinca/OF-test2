...
void testApp::setup(){
    ...
    fingerMovie.loadMovie("movies/redFlashlight.mov");
    
    movWidth = fingerMovie.getWidth();
    cout << "width (x):  " << movWidth << endl;
    movHeight = fingerMovie.getHeight();
    cout << "height(y):  " << movHeight << endl;
    movDuration = fingerMovie.getTotalNumFrames();
    cout << "Duration in frames :" << movDuration << endl;
...
void testApp::draw(){
    ...
    //get all of the pixels in this frame
   ...
   // cout << endl;
    ...
    //my note: i=4 rpresents the first Red pixel in the array of RGB for each frame. This appears o loop through every other RGB array
    ofSetColor(255, 0, 0);
    for (int i = 0; i < movWidth; i+=8){
        for (int j = 0; j < movHeight; j+=8){
            unsigned char r = pixels[(j * movWidth + i)*nChannels];
            //cout << (float)r << "\n";
            if((float)r > 150){
               //cout << "red pixel" << endl;
                font.drawString("Red Pixels!", 20, 480);
            }
            //255 is the value for red in RGB -I think. r is each individual pixel of a frame
